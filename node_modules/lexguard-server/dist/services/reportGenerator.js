...
class ReportGenerator {
    generateHtmlReport(contract, analysis) {
        ...
        const renderRiskCard = (r, badgeColor, badgeBg) => `
    ...
    <table class="table-custom">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <span style="font-size: 11px; padding: 2px 6px; border-radius: 4px; background: ${ev.status === 'critical' ? '#fee2e2; color: #991b1b;' : ev.status === 'milestone' ? '#e0f2fe; color: #0369a1;' : '#f1f5f9; color: #475569;'}">
              ${ev.type.toUpperCase()}
            </span>
          </td>
