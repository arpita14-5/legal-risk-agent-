...
import type { Clause, RiskFinding, RiskCategory, RiskLevel, LegalSourceCitation } from '../types/shared.js';
...
export function detectContractRisks(clauses: Clause[], fullText: string, contractId: string): RiskFinding[] {
  ...
  for (const rule of LEGAL_RISK_RULES) {
    ...
    if (matched) {
      ...
      if (normalizedScore >= 76) level = 'Critical';
      else if (normalizedScore >= 51) level = 'High';
      else if (normalizedScore >= 26) level = 'Moderate';
