import type { RiskFinding, RiskCategory, RiskLevel } from '../types/shared.js';
...
export class RiskScoringEngine {
   ...
   */
  calculateIndividualRiskScore(severity: number, probability: number, impact: number): { score: number; level: RiskLevel } {
   ...
   */
  calculateContractOverallScore(risks: RiskFinding[]): {
    overallScore: number;
    riskLevel: RiskLevel;
    ...
    const categoryRiskLists: Record<RiskCategory, number[]> = {
      Financial: [],
      Legal: [],
      Operational: [],
      Compliance: [],
    ...
    for (const cat of Object.keys(categoryRiskLists) as RiskCategory[]) {
