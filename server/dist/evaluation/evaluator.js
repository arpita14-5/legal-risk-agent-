...
exports.runAcademicEvaluation = runAcademicEvaluation;
const dataset_js_1 = require("./dataset.js");
const clauseClassifier_js_1 = require("../services/clauseClassifier.js");
...
async function runAcademicEvaluation() {
    ...
    for (const item of dataset_js_1.EVALUATION_BENCHMARK_DATASET) {
        const result = (0, clauseClassifier_js_1.classifyClauseText)(item.text);
    ...
    for (const item of dataset_js_1.EVALUATION_BENCHMARK_DATASET) {
        ...
        const detected = (0, riskDetector_js_1.detectContractRisks)([fakeClause], item.text, 'eval-contract');
    ...
    const chunksToSave = dataset_js_1.EVALUATION_BENCHMARK_DATASET.map((item, idx) => ({
    ...
    const retriever = new hybridRetriever_js_1.HybridRetriever();
    let hitsAt3 = 0;
    let hitsAt5 = 0;
    for (const tq of testQueries) {
        const result3 = await retriever.retrieve(testContractId, tq.query, 3);
        const foundAt3 = result3.contractChunks.some(c => c.chunk.text.includes(tq.expectedId) || c.chunk.text === dataset_js_1.EVALUATION_BENCHMARK_DATASET.find(b => b.id === tq.expectedId)?.text);
        if (foundAt3)
            hitsAt3++;
        const result5 = await retriever.retrieve(testContractId, tq.query, 5);
        const foundAt5 = result5.contractChunks.some(c => c.chunk.text.includes(tq.expectedId) || c.chunk.text === dataset_js_1.EVALUATION_BENCHMARK_DATASET.find(b => b.id === tq.expectedId)?.text);
        if (foundAt5) {
            hitsAt5++;
