...
exports.extractContractTimeline = extractContractTimeline;
exports.extractContractObligations = extractContractObligations;
exports.generateExecutiveSummaryData = generateExecutiveSummaryData;
const uuid_1 = require("uuid");
function extractContractMetadata(fullText) {
    const metadata = {
        parties: []
    };
    // Parties detection
    ...
    if (partyMatch) {
        ...
        if (p1.length > 2 && p1.length < 100)
            metadata.parties.push(p1);
        if (p2.length > 2 && p2.length < 100)
            metadata.parties.push(p2);
    }
    else {
        ...
        for (const line of lines) {
            if (line.includes('Inc.') || line.includes('LLC') || line.includes('Corp') || line.includes('Ltd')) {
                ...
                if (!metadata.parties.includes(clean))
                    metadata.parties.push(clean);
            }
        }
    }
    // Effective Date
...
function extractContractTimeline(fullText, clauses, metadata) {
    const events = [];
...
function extractContractObligations(fullText, clauses, metadata) {
    const obligations = [];
...
function generateExecutiveSummaryData(contractType, metadata, risks) {
