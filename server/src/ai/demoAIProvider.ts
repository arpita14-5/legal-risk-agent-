...
export class DemoEmbeddingProvider implements IEmbeddingProvider {
...
export class DemoLLMProvider implements ILLMProvider {
  ...
  async generateText(prompt: string, _systemPrompt?: string): Promise<LLMResponse> {
    // 1. Extract Question
  ...
  async generateStructured<T>(_prompt: string, _systemPrompt?: string): Promise<T> {
    return {} as T;
  }
}
