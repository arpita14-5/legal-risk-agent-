...
export class ComparisonAgent {
  ...
  async compareContracts(
    docA: ContractDocument,
    clausesA: Clause[],
    analysisA: ContractAnalysis,
    docB: ContractDocument,
    clausesB: Clause[],
    ...
    const diffs: ComparisonCategoryDiff[] = [];
    const clauseDiffs: ClauseDiff[] = [];
    const keyDifferencesSummary: string[] = [];
    ...
    for (const cat of categoriesToCompare) {
      ...
      let assessment: 'A safer' | 'B safer' | 'Equal / Mutual' | 'Both risky' = 'Equal / Mutual';
    ...
    const processedTypesInB = new Set<string>();
