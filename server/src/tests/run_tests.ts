...
import { DemoLLMProvider } from '../ai/demoAIProvider.js';
import { cosineSimilarity, getRepository } from '../db/repository.js';
...
async function runAllTests() {
  ...
  await test('JWT signing and payload verification', () => {
    ...
    assert.strictEqual(decoded.email, 'lawyer@firm.com');
  });
  ...
  await test('Categorizes Low Risk correctly (Severity=2, Prob=2, Impact=2)', () => {
    ...
    assert.strictEqual(res.score <= 25, true);
  ...
  await test('Aggregates overall contract score with critical floor guarantee', () => {
    ...
    assert.strictEqual(['Critical', 'High'].includes(overall.riskLevel), true);
    assert.strictEqual(overall.overallScore >= 68, true);
  });
  ...
  await test('Risk detection generates explainable detectionReason, confidence, and suggested counter-draft', () => {
    ...
    const risks = detectContractRisks([fakeClause], clauseText, 'test-contract');
    assert.strictEqual(risks.length > 0, true);
    const liabilityRisk = risks.find(r => r.category === 'Financial');
    assert.strictEqual(!!liabilityRisk, true);
    assert.strictEqual(typeof liabilityRisk!.detectionReason, 'string');
    assert.strictEqual(typeof liabilityRisk!.suggestedClause, 'string');
    assert.strictEqual(typeof liabilityRisk!.negotiationPoint, 'string');
    assert.strictEqual(liabilityRisk!.confidence! >= 0.9, true);
    assert.strictEqual(liabilityRisk!.contractEvidence.section, 'Section 8.1');
    assert.strictEqual(liabilityRisk!.contractEvidence.page, 4);
    assert.strictEqual(liabilityRisk!.score >= 76, true);
  });
  ...
  await test('Extracts comprehensive timeline events and obligations', () => {
    ...
    const meta = extractContractMetadata(sampleText);
    assert.strictEqual(meta.parties.length >= 2, true);
    assert.strictEqual(meta.effectiveDate?.includes('March 1, 2025'), true);

    const timeline = extractContractTimeline(sampleText, [], meta);
    assert.strictEqual(timeline.length >= 4, true);
    assert.strictEqual(timeline.some(t => t.type === 'effective'), true);
    assert.strictEqual(timeline.some(t => t.type === 'renewal'), true);

    const obligations = extractContractObligations(sampleText, [], meta);
    assert.strictEqual(obligations.length >= 3, true);

    const execSummary = generateExecutiveSummaryData('Master Services Agreement', meta, []);
    assert.strictEqual(execSummary.parties.length >= 2, true);
    assert.strictEqual(execSummary.contractType, 'Master Services Agreement');
  });
  ...
  await test('ComparisonAgent identifies added, removed, and modified clauses with risk delta', async () => {
    ...
    const docA: any = { id: 'doc-a', title: 'Contract v1' };
    const docB: any = { id: 'doc-b', title: 'Contract v2' };
    ...
    const analysisA: any = { overallScore: 82, riskLevel: 'Critical' };
    const analysisB: any = { overallScore: 45, riskLevel: 'Moderate' };

    const result = await comparisonAgent.compareContracts(docA, clausesA, analysisA, docB, clausesB, analysisB);
    assert.strictEqual(result.saferContractId, 'doc-b');
    assert.strictEqual(result.riskScoreDelta, -37);
    assert.strictEqual(result.clauseDiffs && result.clauseDiffs.length >= 2, true);

    const modifiedLiability = result.clauseDiffs?.find(d => d.clauseType === 'Liability');
    assert.strictEqual(modifiedLiability?.type, 'modified');
    assert.strictEqual(modifiedLiability?.riskScoreImpact! < 0, true);
  });
  ...
  await test('Repository saves and calculates human feedback summary', async () => {
    ...
    assert.strictEqual(feedbackList.length >= 2, true);

    const summary = await repo.getFeedbackSummary();
    assert.strictEqual(summary.total >= 2, true);
    assert.strictEqual(summary.correct >= 1, true);
    assert.strictEqual(summary.incorrect >= 1, true);
  });
...
Question: What is the liability cap?`;
    ...
    assert.strictEqual(res.content.includes('Section 8.2'), true);
    assert.strictEqual(res.content.includes('Page 4'), true);
    assert.strictEqual(res.content.includes('aggregate liability'), true);
  });
...
Question: Does this contract require compliance with HIPAA healthcare regulations?`;
    ...
    assert.strictEqual(res.content.includes('could not find sufficient evidence'), true);
  });
  ...
  await test('Verification Agent grounds valid claims against retrieved chunks', () => {
    ...
    assert.strictEqual(result.groundedCitations.length > 0, true);
  });
